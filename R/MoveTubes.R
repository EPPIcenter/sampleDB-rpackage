...
MoveTubes <- function(barcode_file, barcode_type, longitudinal, plate_id, location, study_short_code, session){
  ...
  for(i in 1:nrow(csv)){
    ...
    if(csv[i, "individual_id"] %in% sampleDB::CheckTable("study_subject")$uid){

      #IF INDIE ID EXISTS FETCH STUDY_SUBJECT_TABLE_ID ASSO W IT
      study_subject_table_id <- filter(sampleDB::CheckTable("study_subject"), uid == csv[i, ]$"individual_id")$id
      ...
      sampleDB::AddToTable("specimen",
                           list(created = lubridate::now("UTC") %>% as.character(),
                                ...
                                collection_date = dates[i])) #dates need to be in y:m:d format
      ...
      sampleDB::AddToTable("storage_container",
                           list(created = lubridate::now("UTC") %>% as.character(),
                                last_updated = lubridate::now("UTC") %>% as.character(),
                                type = NA,
                                specimen_id = filter(sampleDB::CheckTable("specimen"), study_subject_id == study_subject_table_id, specimen_type_id == filter(table.specimen_type, label == csv[i, ]$"specimen_type")$id)$id,
                                comments = NA,
                                exhausted = 0))
      ...
      sampleDB::AddToTable("study_subject",
                           list(created = lubridate::now("UTC") %>% as.character(),
                                ...
                                study_id = filter(sampleDB::CheckTable("study"), short_code == study_short_code)$id))

      #FETCH THE NEW STUDY_SUBJECT_TABLE_ID
      study_subject_table_id <- tail(sampleDB::CheckTable("study_subject"), 1)$id
      ...
      if(longitudinal == TRUE){
        sampleDB::AddToTable("specimen",
                             list(created = lubridate::now("UTC") %>% as.character(),
                                  ...
                                  study_subject_id = study_subject_table_id,
                                  specimen_type_id = filter(table.specimen_type, label == csv[i,]$"specimen_type")$id,
                                  collection_date = dates[i])) #date is in y:m:d format
      }else{
        sampleDB::AddToTable("specimen",
                             list(created = lubridate::now("UTC") %>% as.character(),
                                  last_updated = lubridate::now("UTC") %>% as.character(),
