...
RenameContainers <- function(sample_type, new_container_name, current_container_name, conn){
  ...
  if (!current_container_name %in% CheckTable(database = database, table = "matrix_plate")$plate_name) {
  ...
  if (new_container_name %in% CheckTable(database = database, table = "matrix_plate")$plate_name) {
