...
UploadSamples <- function(barcode_file, barcode_type, longitudinal, plate_id, location, study_short_code){
  ...
  plate_id <- sampleDB::CheckTable("matrix_plate") %>% tail(1) %>% dplyr::pull(id)
  for(i in 1:nrow(csv)){
  ...
  study_id <- table.study %>% filter(short_code == study_short_code) %>% dplyr::pull(id)

  for(i in 1:nrow(csv)){

    uid <- csv %>% slice(i) %>% dplyr::pull(uid)

    switch.subject_study_id <- FALSE
    if(uid %in% (table.study_subject %>% dplyr::pull(uid))){
      study_subject_id <- table.study_subject %>% filter(uid == uid) %>% dplyr::pull(id)
      ...
      sampleDB::AddToTable("study_subject",
                           list(created = "dummy",
                                ...
                                uid = uid,
                                study_id = study_id))
      study_subject_id <- CheckTable("study_subject") %>% tail(1) %>% dplyr::pull(id)
    }

    specimen_type_id <- table.specimen_type %>% filter(label == csv %>% slice(i) %>% dplyr::pull(specimen_type)) %>% dplyr::pull(id)
    ...
    if(switch.subject_study_id){
      ...
      specimen_id <- table.specimen %>% filter(study_subject_id == study_subject_id, specimen_type_id == specimen_type_id) %>% dplyr::pull(id)
