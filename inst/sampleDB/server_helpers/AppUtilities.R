...
SearchFunction <- function(input, output, ui_elements, user_file){
  ...
  tryCatch(
    ...
    if(input[[ui_elements$ui.input$SubjectUIDSearchType]] == "individual"){
      ...
      list.search_results <- SearchSamples(sample_type = search.type, sample_barcode = search.label, container_name = search.container, study_subject = search.study_subject,
