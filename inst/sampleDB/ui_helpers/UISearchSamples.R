...
UISearchSamples <- function(){
  ...
  ui <- sidebarLayout(
    sidebarPanel(
      ...
      selectizeInput("SearchBySpecimenType", "Specimen Type", choices = c("", CheckTable(database = database, "specimen_type")$name)),
      dateRangeInput("dateRange", label = "Collection Dates", start = NA, end = NA) %>% suppressWarnings(),
      selectizeInput("SearchByLocation", "Storage Location", choices = c("", CheckTable("location")$name)),
