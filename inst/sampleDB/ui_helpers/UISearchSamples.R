...
UISearchSamples <- function(){
  ...
  ui <- sidebarLayout(
    sidebarPanel(
      ...
      radioButtons("SearchBySampleType","Sample Type", choices = c(DBI::dbReadTable(con, "sample_type") %>% pull(id, name = "name")), selected = 1, inline = T),
      hr(),
      ...
      hr(),
      conditionalPanel(condition = "input.SearchBySampleType == \"1\"",
      ...
      conditionalPanel(condition = "input.SearchBySampleType == \"2\"",
