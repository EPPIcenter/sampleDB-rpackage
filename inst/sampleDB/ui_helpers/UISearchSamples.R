...
UISearchSamples <- function(){
  ...
  ui <- sidebarLayout(
    sidebarPanel(
      ...
      hr(),
      selectizeInput("SearchByStudy", "Study", choices = NULL),
      ...
      selectizeInput("SearchBySpecimenType", "Specimen Type", choices = NULL),
      dateRangeInput("dateRange", label = "Collection Dates", start = NA, end = NA) %>% suppressWarnings(),
      selectizeInput("SearchByLocation", "Storage Location", choices = NULL),
      ...
      selectizeInput("SearchByState", "State", choices = DBI::dbReadTable(con, "state")$name),
      selectizeInput("SearchByStatus", "Status", choices = c("In Use"))
    ),
    mainPanel(
      width = 10,
