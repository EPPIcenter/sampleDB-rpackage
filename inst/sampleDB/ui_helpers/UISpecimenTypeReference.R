UISpecimenTypeReference <- function(){
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("RenameSpecimenType1", label = "Current Specimen Type Name", choices = c("", sampleDB::CheckTable(database = database, "specimen_type")$name), selected = 1),
      ...
      selectInput("DeleteSpecimenType", label = NULL, choices = c("", sampleDB::CheckTable(database = database, "specimen_type")$name), selected = 1),
