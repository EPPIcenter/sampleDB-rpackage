UISpecimenTypeReference <- function(){
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("RenameSpecimenType1", label = "Current Specimen Type Name", choices = c("", CheckTable(database = database, "specimen_type")$name), selected = 1),
      ...
      selectInput("DeleteSpecimenType", label = NULL, choices = c("", CheckTable(database = database, "specimen_type")$name), selected = 1),
