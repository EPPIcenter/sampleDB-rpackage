UIFreezerReference <- function(){
  ...
  ui <- sidebarLayout(
    sidebarPanel(
      ...
      selectInput("RenameFreezerName1", label = NULL, choices = c("", CheckTable(database = database, "location")$location_root)),
      selectInput("RenameFreezerLevelI1", label = NULL, choices = c("", CheckTable(database = database, "location")$location_root)),
      selectInput("RenameFreezerLevelII1", label = NULL, choices = c("", CheckTable(database = database, "location")$location_root)),
      ...
      selectInput("DeleteFreezerName", label = NULL, choices = c("", CheckTable(database = database, "location")$location_root)),
