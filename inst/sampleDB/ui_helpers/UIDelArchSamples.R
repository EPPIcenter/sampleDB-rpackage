...
UIDelArchSamples <- function(){
  ...
  ui <- sidebarLayout(
    sidebarPanel(
      ...
      radioButtons("DelArchSearchBySampleType","Sample Type", choices = c(DBI::dbReadTable(con, "sample_type") %>% pull(id, name = "name")), selected = 1, inline = T),
      hr(),
      ...
      hr(),
      conditionalPanel(condition = "input.DelArchSearchBySampleType == \"1\"",
      ...
      conditionalPanel(condition = "input.DelArchSearchBySampleType == \"2\"",
